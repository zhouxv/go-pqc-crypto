...
func parsePublicKey(algo PublicKeyAlgorithm, keyData *publicKeyInfo) (interface{}, error) {
	...
	case PQC:
		...
		// alg := l.GetAlgorithmFromOID(keyData.Algorithm.Algorithm)
		// if alg == pqc.UnknownKeyAlgorithm {
		// 	return nil, errors.New(fmt.Sprintf(
		// 		"unknown PQC public key algorithm with id %s", keyData.Algorithm.Algorithm.String()))
		// }

		// asn1Data := keyData.PublicKey.RightAlign()
		// s := pqc.OQSSigInfo{Algorithm: alg}
		publicKey, err := pqc.ParsePKIXPublicKey(keyData.PublicKey.RightAlign())
		if err != nil {
			return nil, err
		}
